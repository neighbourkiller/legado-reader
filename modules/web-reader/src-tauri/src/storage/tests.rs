...
mod tests {
    ...
    fn test_book_crud_and_derived_columns() {
        ...
        // 2. 读取记录与文件
        let record = db.get_book_record(book_id).unwrap().expect("找不到书籍记录");
    ...
    fn test_reading_records_device_contributions() {
        ...
        // 设备 A 阅读 60 秒
        db.add_reading_time(book_id, "阅读测试书", "作者", 60, 1000, "dev-A").unwrap();
        // 设备 B 阅读 40 秒
        db.add_reading_time(book_id, "阅读测试书", "作者", 40, 2000, "dev-B").unwrap();
    ...
    fn test_preferences_crud_and_delete() {
        ...
        assert_eq!(db.get_preference("test_key").unwrap(), Some("val1".to_string()));
