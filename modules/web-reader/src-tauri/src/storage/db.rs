...
impl StorageDb {
    ...
    pub fn lock(&self) -> Result<std::sync::MutexGuard<'_, Connection>, StorageErrorPayload> {
        self.conn.lock().map_err(|e| {
            StorageErrorPayload::new("TRANSACTION", "acquire_lock", format!("数据库互斥锁中毒: {e}"))
        })
    }
    ...
    pub fn save_book(
        ...
        let meta_str = serde_json::to_string(meta_json)
            .map_err(|e| StorageErrorPayload::new("INVALID_DATA", "serialize_meta", e.to_string()))?;
        let chapters_str = serde_json::to_string(chapters_json)
            .map_err(|e| StorageErrorPayload::new("INVALID_DATA", "serialize_chapters", e.to_string()))?;
    ...
    pub fn get_book_record(&self, id: &str) -> Result<Option<StoredBookRecord>, StorageErrorPayload> {
        let conn = self.lock()?;
        let mut stmt = conn
            .prepare(
        ...
        if let Some((meta_str, chapters_str, has_file_data, file_size)) = record {
            let meta: Value = serde_json::from_str(&meta_str)
                .map_err(|e| StorageErrorPayload::new("INVALID_DATA", "parse_meta", e.to_string()))?;
            let chapters: Value = serde_json::from_str(&chapters_str)
                .map_err(|e| StorageErrorPayload::new("INVALID_DATA", "parse_chapters", e.to_string()))?;
    ...
    pub fn get_all_book_metas(&self) -> Result<Vec<Value>, StorageErrorPayload> {
        ...
        for item in rows {
            let s = item.map_err(|e| StorageErrorPayload::new("IO", "read_meta_row", e.to_string()))?;
            let v: Value = serde_json::from_str(&s)
                .map_err(|e| StorageErrorPayload::new("INVALID_DATA", "parse_meta_row", e.to_string()))?;
            metas.push(v);
        }
        Ok(metas)
    }
    ...
    pub fn update_book_meta(&self, id: &str, updates: &Value) -> Result<(), StorageErrorPayload> {
        ...
        let meta_str: Option<String> = conn
            .query_row("SELECT meta_json FROM books WHERE id = ?1", params![id], |row| row.get(0))
            .optional()
            .map_err(|e| StorageErrorPayload::new("IO", "get_book_meta_for_update", e.to_string()))?;
        ...
        let mut meta_val: Value = serde_json::from_str(&meta_str)
            .map_err(|e| StorageErrorPayload::new("INVALID_DATA", "parse_existing_meta", e.to_string()))?;
        ...
        let new_meta_str = serde_json::to_string(&meta_val)
            .map_err(|e| StorageErrorPayload::new("INVALID_DATA", "serialize_updated_meta", e.to_string()))?;
    ...
    pub fn delete_book(&self, id: &str) -> Result<(), StorageErrorPayload> {
        ...
        let tx = conn
            .transaction()
            .map_err(|e| StorageErrorPayload::new("TRANSACTION", "begin_delete_book", e.to_string()))?;
    ...
    pub fn get_all_stored_book_files(&self) -> Result<Vec<StoredBookFileInfo>, StorageErrorPayload> {
        let conn = self.lock()?;
        let mut stmt = conn
            .prepare(
            ...
            .map_err(|e| StorageErrorPayload::new("IO", "prepare_get_stored_files", e.to_string()))?;
        ...
        for f in rows {
            files.push(f.map_err(|e| StorageErrorPayload::new("IO", "read_stored_file", e.to_string()))?);
        }
        Ok(files)
    }
    ...
    pub fn get_all_bookmarks(&self) -> Result<Vec<BookmarkRecord>, StorageErrorPayload> {
        ...
        for b in rows {
            bookmarks.push(b.map_err(|e| StorageErrorPayload::new("IO", "read_bookmark", e.to_string()))?);
        }
        Ok(bookmarks)
    }

    pub fn get_bookmarks_by_book_id(&self, book_id: &str) -> Result<Vec<BookmarkRecord>, StorageErrorPayload> {
        let conn = self.lock()?;
        let mut stmt = conn
            .prepare(
                "SELECT id, book_id, book_name, book_author, chapter_index, chapter_pos,
        ...
        let mut bookmarks = Vec::new();
        for b in rows {
            bookmarks.push(b.map_err(|e| StorageErrorPayload::new("IO", "read_book_bookmark", e.to_string()))?);
        }
        Ok(bookmarks)
    }
    ...
    pub fn save_highlight(&self, highlight: &HighlightRecord) -> Result<(), StorageErrorPayload> {
        let conn = self.lock()?;
        let style_str = serde_json::to_string(&highlight.style)
            .map_err(|e| StorageErrorPayload::new("INVALID_DATA", "serialize_highlight_style", e.to_string()))?;
    ...
    pub fn get_highlights_by_book_id(&self, book_id: &str) -> Result<Vec<HighlightRecord>, StorageErrorPayload> {
        let conn = self.lock()?;
        let mut stmt = conn
            .prepare(
            ...
            .map_err(|e| StorageErrorPayload::new("IO", "prepare_get_book_highlights", e.to_string()))?;
        ...
        for h in rows {
            list.push(h.map_err(|e| StorageErrorPayload::new("IO", "read_highlight", e.to_string()))?);
        }
        Ok(list)
    }

    pub fn get_highlights_by_chapter(&self, book_id: &str, chapter_index: i64) -> Result<Vec<HighlightRecord>, StorageErrorPayload> {
        let conn = self.lock()?;
        let mut stmt = conn
            .prepare(
                "SELECT id, book_id, book_name, book_author, book_url, chapter_url, chapter_index,
            ...
            .map_err(|e| StorageErrorPayload::new("IO", "prepare_get_chapter_highlights", e.to_string()))?;
        ...
        let rows = stmt
            ...
            .map_err(|e| StorageErrorPayload::new("IO", "query_chapter_highlights", e.to_string()))?;

        let mut list = Vec::new();
        for h in rows {
            list.push(h.map_err(|e| StorageErrorPayload::new("IO", "read_chapter_highlight", e.to_string()))?);
        }
        Ok(list)
    }
    ...
    pub fn get_all_replace_rules(&self) -> Result<Vec<ReplaceRuleRecord>, StorageErrorPayload> {
        ...
        let mut stmt = conn
            ...
            .map_err(|e| StorageErrorPayload::new("IO", "prepare_get_replace_rules", e.to_string()))?;
        ...
        let mut list = Vec::new();
        for r in rows {
            list.push(r.map_err(|e| StorageErrorPayload::new("IO", "read_replace_rule", e.to_string()))?);
        }
        Ok(list)
    }
    ...
    pub fn add_reading_time(
        ...
        let existing: Option<(i64, i64, String)> = conn
            ...
            .optional()
            .map_err(|e| StorageErrorPayload::new("IO", "query_existing_reading_record", e.to_string()))?;

        let mut devices: HashMap<String, ReadingDeviceContribution> = if let Some((_, _, dev_str)) = &existing {
            serde_json::from_str(dev_str).unwrap_or_default()
        } else {
            HashMap::new()
        };

        let current_dev = devices.entry(device_id.to_string()).or_insert_with(|| ReadingDeviceContribution {
            read_time: 0,
            last_read: timestamp,
        ...
        let max_last_read: i64 = devices.values().map(|d| d.last_read).max().unwrap_or(timestamp);

        let dev_json_str = serde_json::to_string(&devices)
            .map_err(|e| StorageErrorPayload::new("INVALID_DATA", "serialize_devices", e.to_string()))?;
    ...
    pub fn get_all_reading_records(&self) -> Result<Vec<ReadingRecord>, StorageErrorPayload> {
        ...
        let mut stmt = conn
            ...
            .map_err(|e| StorageErrorPayload::new("IO", "prepare_get_reading_records", e.to_string()))?;
        ...
        let mut list = Vec::new();
        for r in rows {
            list.push(r.map_err(|e| StorageErrorPayload::new("IO", "read_reading_record", e.to_string()))?);
        }
        Ok(list)
    }
    ...
    pub fn delete_reading_record(&self, book_id: &str) -> Result<(), StorageErrorPayload> {
        let conn = self.lock()?;
        conn.execute("DELETE FROM reading_records WHERE book_id = ?1", params![book_id])
    ...
    pub fn save_chapter_content(&self, content: &StoredChapterContent) -> Result<(), StorageErrorPayload> {
        let conn = self.lock()?;
        // 计算 UTF-8 JSON 字节数，保持与 TextEncoder(JSON.stringify(record)) 完全一致的统计口径
        let record_bytes = serde_json::to_vec(content)
            .map_err(|e| StorageErrorPayload::new("INVALID_DATA", "serialize_chapter_content", e.to_string()))?;
    ...
    pub fn get_chapter_content(
        ...
        let mut stmt = conn
            ...
            .map_err(|e| StorageErrorPayload::new("IO", "prepare_get_chapter_content", e.to_string()))?;
    ...
    pub fn get_book_chapter_contents(&self, book_id: &str) -> Result<Vec<StoredChapterContent>, StorageErrorPayload> {
        let conn = self.lock()?;
        let mut stmt = conn
            .prepare(
        ...
        let mut list = Vec::new();
        for c in rows {
            list.push(c.map_err(|e| StorageErrorPayload::new("IO", "read_book_chapter", e.to_string()))?);
        }
        Ok(list)
    }

    pub fn get_chapter_cache_summaries(&self) -> Result<Vec<ChapterCacheSummary>, StorageErrorPayload> {
        let conn = self.lock()?;
        let mut stmt = conn
            .prepare(
        ...
        for s in rows {
            summaries.push(s.map_err(|e| StorageErrorPayload::new("IO", "read_cache_summary", e.to_string()))?);
        }
        Ok(summaries)
    }
    ...
    pub fn save_book_source(&self, source: &Value) -> Result<(), StorageErrorPayload> {
        let url = source["bookSourceUrl"]
            .as_str()
            .ok_or_else(|| StorageErrorPayload::new("INVALID_DATA", "save_book_source", "缺少 bookSourceUrl"))?;
        ...
        let data_str = serde_json::to_string(source)
            .map_err(|e| StorageErrorPayload::new("INVALID_DATA", "serialize_book_source", e.to_string()))?;
    ...
    pub fn get_all_book_sources(&self) -> Result<Vec<Value>, StorageErrorPayload> {
        ...
        let mut stmt = conn
            ...
            .map_err(|e| StorageErrorPayload::new("IO", "prepare_get_book_sources", e.to_string()))?;
        ...
        let mut list = Vec::new();
        for item in rows {
            let s = item.map_err(|e| StorageErrorPayload::new("IO", "read_source_row", e.to_string()))?;
            let v: Value = serde_json::from_str(&s)
                .map_err(|e| StorageErrorPayload::new("INVALID_DATA", "parse_source_row", e.to_string()))?;
            list.push(v);
        }
        Ok(list)
    }
    ...
    pub fn delete_book_source(&self, book_source_url: &str) -> Result<(), StorageErrorPayload> {
        let conn = self.lock()?;
        conn.execute("DELETE FROM book_sources WHERE book_source_url = ?1", params![book_source_url])
    ...
    pub fn import_book_sources(&self, sources: &[Value]) -> Result<usize, StorageErrorPayload> {
        ...
        let tx = conn
            .transaction()
            .map_err(|e| StorageErrorPayload::new("TRANSACTION", "begin_import_sources", e.to_string()))?;
        ...
        let mut count = 0;
        {
            let mut stmt = tx
                ...
                .map_err(|e| StorageErrorPayload::new("IO", "prepare_import_source", e.to_string()))?;
            ...
            for source in sources {
                if let Some(url) = source["bookSourceUrl"].as_str() {
                    ...
                    .map_err(|e| StorageErrorPayload::new("IO", "exec_import_source", e.to_string()))?;
                    count += 1;
                }
            }
        }
        ...
        tx.commit()
            .map_err(|e| StorageErrorPayload::new("TRANSACTION", "commit_import_sources", e.to_string()))?;
    ...
    pub fn save_remote_book(&self, id: &str, data: &Value) -> Result<(), StorageErrorPayload> {
        let data_str = serde_json::to_string(data)
            .map_err(|e| StorageErrorPayload::new("INVALID_DATA", "serialize_remote_book", e.to_string()))?;
        let conn = self.lock()?;
        conn.execute(
    ...
    pub fn get_all_remote_books(&self) -> Result<Vec<Value>, StorageErrorPayload> {
        ...
        let mut stmt = conn
            ...
            .map_err(|e| StorageErrorPayload::new("IO", "prepare_get_remote_books", e.to_string()))?;
        ...
        let mut list = Vec::new();
        for item in rows {
            let s = item.map_err(|e| StorageErrorPayload::new("IO", "read_remote_book_row", e.to_string()))?;
            let v: Value = serde_json::from_str(&s)
                .map_err(|e| StorageErrorPayload::new("INVALID_DATA", "parse_remote_book_row", e.to_string()))?;
            list.push(v);
        }
        Ok(list)
    }
    ...
    pub fn get_preference(&self, key: &str) -> Result<Option<String>, StorageErrorPayload> {
        let conn = self.lock()?;
        let res: Option<String> = conn
            .query_row("SELECT value FROM app_preferences WHERE key = ?1", params![key], |row| row.get(0))
            .optional()
    ...
    pub fn get_all_preferences(&self) -> Result<HashMap<String, String>, StorageErrorPayload> {
        ...
        for item in rows {
            let (k, v) = item.map_err(|e| StorageErrorPayload::new("IO", "read_pref_row", e.to_string()))?;
            map.insert(k, v);
        }
        Ok(map)
    }
