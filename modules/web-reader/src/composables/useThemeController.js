...
import { resolveEffectiveDark, useTheme, } from '@/composables/useTheme';
export function resolveThemeChangeDisposition(preference, currentDark, targetDark) {
    if (currentDark === targetDark || preference === 'independent')
        return 'apply';
...
export function useThemeController() {
    ...
    const syncReadingTheme = async (dark) => {
        try {
    ...
    const requestTheme = async (mode) => {
        const targetDark = resolveEffectiveDark(mode, systemPrefersDark.value);
        const disposition = resolveThemeChangeDisposition(appSettings.readerThemeSyncPreference, isDark.value, targetDark);
        ...
        applyTheme(mode);
        if (disposition === 'sync')
            await syncReadingTheme(targetDark);
    };
    const chooseSync = async () => {
        ...
        if (!change)
            return;
        if (rememberChoice)
            appSettings.setReaderThemeSyncPreference('sync');
    ...
    const chooseIndependent = () => {
        ...
        if (!change)
            return;
        if (rememberChoice)
            appSettings.setReaderThemeSyncPreference('independent');
    };
    const closePrompt = () => {
        pendingChange = null;
        prompt.visible = false;
