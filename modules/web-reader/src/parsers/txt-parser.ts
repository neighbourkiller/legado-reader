import type { BookChapter, BookMeta, ParsedBook } from './types'
 ...
 */
function detectChapterPattern(text: string): RegExp | null {
...
function splitIntoChapters(text: string): BookChapter[] {
...
function splitByPattern(text: string, pattern: RegExp): BookChapter[] {
  const globalPattern = new RegExp(pattern.source, 'gm')
...
function splitBySize(text: string): BookChapter[] {
  const chapters: BookChapter[] = []
...
const textCache = new WeakMap<ArrayBuffer, string>()
 ...
 */
function parseNameAuthor(fileName: string): { name: string; author: string } {
