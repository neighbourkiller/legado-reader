import type { BookChapter, StoredBook } from '@/parsers/types'
...
import type {
  ...
  AndroidBook,
  AndroidBookmark,
  AndroidBookHighlight,
  AndroidConversionContext,
  AndroidReadRecord,
} from './types'

const ANDROID_LOCAL_ORIGIN = 'LOCAL'

function cssColorToAndroidArgb(color: string): number {
  ...
  let alpha = 128
  if (rgba) {
    red = Number(rgba[1]); green = Number(rgba[2]); blue = Number(rgba[3])
...
function androidArgbToCss(value: number, includeAlpha: boolean): string {
...
function toAndroidHighlightStyle(style: HighlightStyleRecord): string {
...
function fromAndroidHighlightStyle(raw: string): HighlightStyleRecord {
  try {
    const parsed = JSON.parse(raw) as { fill?: number; underline?: { kind?: string; color?: number } }
    if (parsed.underline) {
      return {
        kind: 'underline',
    ...
    if (parsed.fill) return { kind: 'background', color: androidArgbToCss(parsed.fill, true) }
  } catch {
...
function clampInteger(value: unknown, minimum = 0): number {
...
export function paragraphIndexToCharacterOffset(content: string, paragraphIndex: number): number {
...
export function characterOffsetToParagraphIndex(
  content: string,
  ...
  for (let index = 0; index < paragraphs.length; index += 1) {
    ...
    if (characterOffset < nextOffset) return index
    offset = nextOffset
  }
  return paragraphs.length - 1
}

function chapterContentFor(
  bookId: string,
  chapterIndex: number,
  contents: StoredChapterContent[],
): string | undefined {
  return contents.find(item => item.bookId === bookId && item.chapterIndex === chapterIndex)?.content
}

function contentWithoutRepeatedTitle(record: StoredChapterContent | undefined): string | undefined {
...
export function fromAndroidBook(book: AndroidBook, existing?: StoredBook): StoredBook | null {
  const bookUrl = asText(book.bookUrl).trim()
  const origin = asText(book.origin).trim()
  const name = asText(book.name).trim()
  if (!bookUrl || !origin || !name || origin.toUpperCase() === ANDROID_LOCAL_ORIGIN) return null
...
export function toAndroidBookmark(
  bookmark: BookmarkRecord,
  chapterContents: StoredChapterContent[],
): AndroidBookmark {
  const chapterRecord = chapterContents.find(
    item => item.bookId === bookmark.bookId && item.chapterIndex === bookmark.chapterIndex,
  )
  const chapterContent = contentWithoutRepeatedTitle(chapterRecord)
...
export function fromAndroidBookmark(
  bookmark: AndroidBookmark,
  books: StoredBook[],
  chapterContents: StoredChapterContent[],
): BookmarkRecord | null {
  const book = books.find(item =>
    item.meta.name === bookmark.bookName &&
...
export function toAndroidHighlight(highlight: HighlightRecord): AndroidBookHighlight {
  return {
...
export function fromAndroidHighlight(
  highlight: AndroidBookHighlight,
  books: StoredBook[],
  chapterContents: StoredChapterContent[] = [],
): HighlightRecord | null {
  const book = books.find(item =>
    (highlight.bookUrl && item.meta.bookUrl === highlight.bookUrl) ||
  ...
  const content = contentWithoutRepeatedTitle(chapterContents.find(item =>
    item.bookId === book.meta.id && item.chapterIndex === clampInteger(highlight.chapterIndex),
  ))
...
export function toAndroidReadRecords(records: ReadingRecord[]): AndroidReadRecord[] {
  const grouped = new Map<string, AndroidReadRecord & { authors: Set<string> }>()
...
export function fromAndroidReadRecords(
  androidRecords: AndroidReadRecord[],
  books: StoredBook[],
  ...
  for (const item of androidRecords) {
    ...
    const book = books.find(candidate =>
      candidate.meta.name === item.bookName &&
      (acceptedAuthors.size === 0 || acceptedAuthors.has(candidate.meta.author)),
    )
    if (!book || !item.deviceId) continue

    const current = records.get(book.meta.id)
    const devices: Record<string, ReadingDeviceContribution> = {
      ...(current?.devices || {}),
      [item.deviceId]: {
...
function decodeAndroidAuthors(author: string): Set<string> {
  const prefix = '\u001Eauthors:'
  if (!author) return new Set()
  if (!author.startsWith(prefix)) return new Set([author])
  try {
...
export function createAndroidBackupData(context: AndroidConversionContext): {
  data: AndroidBackupData
  positionFallbacks: number
} {
  const onlineBooks: AndroidBook[] = []
  ...
  for (const stored of context.books) {
    ...
    if (converted.positionFallback) positionFallbacks += 1
  }

  const onlineBookIds = new Set(
    context.books.filter(book => book.meta.format === 'online').map(book => book.meta.id),
  )
  return {
    positionFallbacks,
    data: {
      ...
      readingRecords: toAndroidReadRecords(
        context.readingRecords.filter(record => onlineBookIds.has(record.bookId)),
      ),
...
export function placeholderChapters(book: AndroidBook): BookChapter[] {
