...
export type DownloadStatus = 'running' | 'completed' | 'partial' | 'cancelled'
...
export const useDownloadStore = defineStore('download', () => {
  const tasks = ref<Record<string, NovelDownloadTask>>({})
  const cancelFlags = new Map<string, { cancelled: boolean }>()

  const activeCount = computed(
    () => Object.values(tasks.value).filter(task => task.status === 'running').length,
  )

  function getTask(bookId: string): NovelDownloadTask | undefined {
  ...
  async function getDownloadedCount(
    ...
    return records.filter(record => {
      if (sourceUrl && record.sourceUrl && record.sourceUrl !== sourceUrl) return false
      const chapter = chapters?.[record.chapterIndex]
      if (chapters && !chapter) return false
      if (chapter?.href && record.chapterUrl && record.chapterUrl !== chapter.href) return false
      return true
    }).length
  }
  ...
  async function startDownload(
  ...
  ): Promise<NovelDownloadTask> {
    if (chapters.length === 0) throw new Error('当前书籍没有可下载的章节')
    ...
    const cachedIndexes = new Set(
      existing
        .filter(record => {
          if (record.sourceUrl && record.sourceUrl !== source.bookSourceUrl) return false
          const chapter = chapters[record.chapterIndex]
    ...
    const worker = async () => {
      ...
      while (!cancelFlag.cancelled) {
        ...
        try {
          ...
          if (!cancelFlag.cancelled) activeTask.completed += 1
        }
      }
    }
    ...
    try {
      await Promise.all(
        Array.from(
          { length: Math.min(DOWNLOAD_CONCURRENCY, Math.max(1, pending.length)) },
          () => worker(),
        ),
      )
  ...
  function cancelDownload(bookId: string) {
