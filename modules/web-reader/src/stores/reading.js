...
import { getBook, getChapterContent, getBookmarksByBookId, saveBook, saveBookmark, saveChapterContent, updateBookMeta, saveSettings, loadSettings, } from '@/storage/db';
...
export function resolveSyncedReaderTheme(currentTheme, dark) {
    if (dark)
        return 6;
    return currentTheme === 6 ? 1 : currentTheme;
}
const SETTINGS_KEY = 'legado_web_reader_settings';
function normalizeSettings(...sources) {
    const availableSources = sources.filter((source) => Boolean(source));
    return {
...
async function resolveImportedBookmarkPositions(bookId, chapterIndex, content) {
    const bookmarks = await getBookmarksByBookId(bookId);
    const unresolved = bookmarks.filter(bookmark => bookmark.chapterIndex === chapterIndex && bookmark.androidChapterPos !== undefined);
    for (const bookmark of unresolved) {
        const chapterPos = characterOffsetToParagraphIndex(content, bookmark.androidChapterPos || 0, bookmark.content);
...
export const useReadingStore = defineStore('reading', () => {
    ...
    const settings = ref(normalizeSettings(loadLocalSettings()));
    ...
    const progress = computed(() => {
        if (!currentBook.value || currentBook.value.totalChapters === 0)
            return 0;
        return Math.round(((currentBook.value.currentChapter + 1) / currentBook.value.totalChapters) * 100);
    });
            ...
            if (stored.meta.format === 'online' && stored.chapters.length === 0) {
                ...
                try {
                    ...
                    if (info.tocUrl)
                        tocUrl = info.tocUrl;
                    if (info.coverUrl)
                        stored.meta.coverUrl = info.coverUrl;
                    if (info.intro)
                        stored.meta.intro = info.intro;
                }
                catch (error) {
                ...
                stored.meta.currentChapter = Math.min(Math.max(0, stored.meta.currentChapter), stored.chapters.length - 1);
    ...
    async function fetchChapter(index, options = {}) {
        ...
        if (currentBook.value.format === 'online') {
            ...
            if (!options.forceRefresh) {
                ...
                if (cached) {
                    await resolveImportedBookmarkPositions(currentBook.value.id, index, normalizeOnlineContent(cached.content, chapter.title).join('\n'));
                    return {
                        index,
                        title: chapter.title,
            ...
            try {
                ...
                await resolveImportedBookmarkPositions(currentBook.value.id, index, normalizeOnlineContent(rawContent, chapter.title).join('\n'));
                return {
                    index,
                    title: chapter.title,
    ...
    async function loadChapter(index) {
        if (!currentBook.value || !fileData.value)
            return;
        if (index < 0 || index >= chapters.value.length)
            return;
        isLoading.value = true;
    ...
    async function saveProgress(chapterIndex, chapterPos) {
        if (!currentBook.value)
            return;
        ...
        currentBook.value.currentProgress = Math.round(((targetIndex + 1) / (chapters.value.length || 1)) * 100);
        currentBook.value.lastReadTime = Date.now();
    ...
    async function syncThemeWithGlobal(dark) {
        await hydrateSettings();
