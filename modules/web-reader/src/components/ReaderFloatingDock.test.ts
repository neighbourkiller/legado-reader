...
describe('阅读器沉浸式浮岛控制栏测试', () => {
  it('ReaderFloatingDock.vue 组件文件存在且包含高频控制与更多功能菜单', () => {
    ...
    expect(content).toContain('--reader-dock-position-offset')
    expect(content).toContain('height: 64px')
    expect(content).not.toContain('--reader-dock-item-height')
    expect(content).not.toContain('--reader-dock-icon-size')
