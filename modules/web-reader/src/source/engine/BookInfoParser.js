...
export function parseBookInfo(html, rule, baseUrl) {
    ...
    if (!rawTocUrl && !isJson && context instanceof Document) {
        rawTocUrl = parseString(context, 'a[href*="chapter"]@href || a[href*="mulu"]@href || a:contains("目录")@href || a:contains("章节")@href');
    }
    ...
    if (!rawCoverUrl && !isJson && context instanceof Document) {
        rawCoverUrl = parseString(context, 'meta[property="og:image"]@content || meta[name="twitter:image"]@content || img.lazyload_book_cover@src || img.lazyload_book_cover@data-src || .fengmian2@src || .fengmian2@data-src || .cover img@src || .book-cover img@src || img@src');
    }
