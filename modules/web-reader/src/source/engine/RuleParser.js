 ...
 * 生成稳定的网络书籍唯一 ID
 */
export function generateBookId(name, author = '', sourceUrl = '') {
...
export function parseRuleType(rule) {
 ...
 */
export function parseList(content, rule, isJson = false) {
    if (!rule)
        return [];
 ...
 */
export function parseString(context, rule) {
    if (!rule || !context)
        return '';
    const subRules = rule.split('||').map(r => r.trim()).filter(Boolean);
    for (const singleRule of subRules) {
...
function parseSingleRuleString(context, rule) {
    let mainRule = rule.trim();
    let regexReplace = null;
    ...
    // 2. DOM Node / Element 提取
        ...
        extracted = extractDomString(context, mainRule);
    }
    // 3. 纯文本上下文
...
function extractDomString(context, rule) {
    if (!rule) {
    ...
    if (trimmed.includes('@') &&
        ...
        try {
            ...
            if (targetEl) {
                ...
                if (attr === 'src') {
                    return (targetEl.getAttribute('src') ||
